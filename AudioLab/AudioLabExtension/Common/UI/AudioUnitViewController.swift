...
private let log = Logger(subsystem: "com.carlosreinoso.AudioLabExtension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<AudioLabExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try AudioLabExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? AudioLabExtensionAudioUnit else {
				log.error("Unable to create AudioLabExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(AudioLabExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = AudioLabExtensionMainView(parameterTree: observableParameterTree)
